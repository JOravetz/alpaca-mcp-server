...
int get_stock_snapshots(char **symbols, int symbol_count, StockData *results, int *result_count, double max_price, double min_percent_change, int min_trades, int all_symbols, int raw_output) {
    ...
    *result_count = 0;
    ...
    for (int i = 0; i < symbol_count; i++) {
        ...
        // IMPORTANT: Market session detection:
        // - During regular trading hours: dailyBar is TODAY's bar (incomplete)
        //   Use prevDailyBar.c as reference (previous completed day's close)
        // - During pre-market/after-hours: dailyBar is YESTERDAY's bar (complete)
        //   Use dailyBar.c as reference (yesterday's close)
        //
        // We detect pre/after market by checking if dailyBar timestamp is from today.
        // If dailyBar is from a previous date, we're in pre/after market.
        
        // Extract dailyBar timestamp to check if it's from today
        json_object *daily_bar_timestamp;
        int is_premarket_or_afterhours = 0;
        
        if (json_object_object_get_ex(daily_bar, "t", &daily_bar_timestamp)) {
            const char *daily_ts = json_object_get_string(daily_bar_timestamp);
            const char *trade_ts = timestamp_json ? json_object_get_string(timestamp_json) : "";
            
            // Compare dates (first 10 chars: YYYY-MM-DD)
            if (strlen(daily_ts) >= 10 && strlen(trade_ts) >= 10) {
                if (strncmp(daily_ts, trade_ts, 10) != 0) {
                    // dailyBar date != latestTrade date, so we're in pre/after market
                    is_premarket_or_afterhours = 1;
                }
            }
        }
        
        if (is_premarket_or_afterhours) {
            // Pre-market or after-hours: use yesterday's close (dailyBar.c)
            reference_price = day_close;
        } else if (prev_daily_bar) {
            // Regular trading hours: use previous day's close (prevDailyBar.c)
            ...
            reference_price = day_close; // Fallback for newly listed stocks
        }
...
char **g_sort_keys = NULL;
int g_sort_key_count = 0;

// Parse sort keys from comma-separated string
void parse_sort_keys(const char *keys_str) {
    ...
    // Count keys first
    g_sort_key_count = 1;
    for (const char *p = keys_str; *p; p++) {
        if (*p == ',') g_sort_key_count++;
    }

    // Allocate array for keys
    g_sort_keys = malloc(g_sort_key_count * sizeof(char*));
    if (!g_sort_keys) {
        free(str_copy);
        return;
    }
    ...
    int i = 0;
    char *token = strtok(str_copy, ",");
    while (token && i < g_sort_key_count) {
        // Trim whitespace
        while (*token && isspace(*token)) token++;
        char *end = token + strlen(token) - 1;
        while (end > token && isspace(*end)) *end-- = '\0';

        g_sort_keys[i] = strdup(token);
        i++;
        token = strtok(NULL, ",");
    }
...
double get_sort_value(const StockData *stock, const char *key) {
    if (strcmp(key, "symbol") == 0) {
        // For symbol, use ASCII value of first character
        return (double)stock->symbol[0];
    } else if (strcmp(key, "price") == 0) {
        return stock->price;
    } else if (strcmp(key, "day_close") == 0) {
        return stock->day_close;
    } else if (strcmp(key, "percent_change") == 0) {
        return stock->percent;
    } else if (strcmp(key, "gradient_full") == 0) {
        return stock->gradient_full;
    } else if (strcmp(key, "gradient_recent") == 0) {
        return stock->gradient_recent;
    } else if (strcmp(key, "gradient_change") == 0) {
        return stock->gradient_change;
    } else if (strcmp(key, "volume") == 0) {
        return (double)stock->volume;
    } else if (strcmp(key, "volume_change") == 0) {
        return (double)stock->volume_change;
    } else if (strcmp(key, "trades") == 0) {
        return (double)stock->trades;
    } else if (strcmp(key, "trades_change") == 0) {
        return (double)stock->trades_change;
    }
    return 0.0;
}

// Multi-key comparison function (ascending order)
int compare_stocks(const void *a, const void *b) {
    ...
    for (int i = 0; i < g_sort_key_count; i++) {
        if (!g_sort_keys[i]) continue;

        double val_a = get_sort_value(stock_a, g_sort_keys[i]);
        double val_b = get_sort_value(stock_b, g_sort_keys[i]);
...
void generate_json_output(StockData *results, int count, int max_output, int all_symbols, const char *sort_keys) {
    // Parse and set up sort keys
    parse_sort_keys(sort_keys);

    // Sort using multi-key comparison
    qsort(results, count, sizeof(StockData), compare_stocks);
    ...
    if (g_sort_keys) free(g_sort_keys);
    g_sort_keys = NULL;
    g_sort_key_count = 0;
}
...
void print_help(const char *program_name) {
    ...
    printf("  -k KEYS       Comma-separated sort keys in ascending order (default: trades,percent_change)\n");
    printf("                Available keys: symbol, price, day_close, percent_change, gradient_full,\n");
    printf("                gradient_recent, gradient_change, volume, volume_change, trades, trades_change\n");
...
int main(int argc, char *argv[]) {
    ...
    const char *sort_keys = "trades,percent_change";  // Default sorting keys
    ...
    generate_json_output(results, result_count, max_output, all_symbols, sort_keys);
