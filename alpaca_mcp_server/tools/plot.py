...
def setup_backend_for_args(no_plot=False):
    ...
    # Always use Agg backend to avoid display issues
    matplotlib.use("Agg", force=True)
    INTERACTIVE_BACKEND = False
...
class MCPPlotManager:
    ...
    def save_and_display(self, fig, filepath):
        try:
            if INTERACTIVE_BACKEND:
                ...
                # For non-interactive backends, save to file then display with ImageMagick
                logger.info(f"Saving plot to {filepath}")
                fig.savefig(
                    filepath, dpi=100, bbox_inches="tight", facecolor="#212529", edgecolor="none"
                )
                plt.close(fig)  # Close figure to free memory

                # Display with ImageMagick in background
                self._display_with_imagemagick(filepath)
    ...
    def _display_with_imagemagick(self, filepath):
        """Display image using ImageMagick in background process"""
        try:
            # Use ImageMagick's display command
            process = subprocess.Popen(
                ["display", filepath], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            self.display_processes.append(process)
            logger.info(f"Started ImageMagick display process {process.pid} for {filepath}")

            # Schedule cleanup when display window closes
            self._schedule_cleanup_on_close(process)

        except FileNotFoundError:
            logger.warning(
                "ImageMagick 'display' command not found. Install with: sudo apt install imagemagick"
            )
        except Exception as e:
            logger.error(f"Failed to display with ImageMagick: {e}")
...
def create_market_calendar_dataframe(timestamps, timeframe):
    ...
    for date, _row in trading_days.iterrows():
        ...
        market_open = nyc_tz.localize(
            datetime.combine(trading_date, datetime.min.time().replace(hour=4))
        )
        market_close = nyc_tz.localize(
            datetime.combine(trading_date, datetime.min.time().replace(hour=20))
        )
    ...
    if extended_schedule:
        market_df = pd.DataFrame({"timestamp": extended_schedule, "close": np.nan})
        market_df.set_index("timestamp", inplace=True)
        market_df.sort_index(inplace=True)

        logger.info(
            f"Created market calendar DataFrame with {len(market_df)} time points from {market_df.index[0]} to {market_df.index[-1]}"
        )
        return market_df
    else:
...
def create_simple_uniform_grid(timestamps, timeframe):
    ...
    uniform_times = pd.date_range(start=start_time, end=end_time, freq=f"{interval_minutes}min")

    # Convert to DataFrame
    market_df = pd.DataFrame({"timestamp": uniform_times, "close": np.nan})
    market_df.set_index("timestamp", inplace=True)
...
def interpolate_to_uniform_grid(timestamps, prices, timeframe):
    ...
    # Create market calendar DataFrame
    ...
    original_df = pd.DataFrame(
        {"timestamp": [dt.astimezone(nyc_tz) for dt in original_times], "close": prices}
    )
    ...
    logger.info(
        f"Interpolated {len(original_times)} irregular samples to {len(market_df)} uniform market-hours samples"
    )
        ...
        try:
            ...
            while page_count < max_pages:
                ...
                if next_page_token:
                    ...
                    logger.debug(
                        f"Fetching page {page_count} with token: {next_page_token[:20]}..."
                    )
...
def process_bars_for_peaks(
    ...
    try:
        ...
        # Use raw data directly without interpolation
        # Convert timestamps to datetime objects for display
        datetime_objects = [parse_timestamp_to_datetime(ts) for ts in timestamps_raw]
        datetime_lookup = dict(enumerate(datetime_objects))

        # Create formatted time strings for display
        formatted_times = [format_datetime_for_display(dt) for dt in datetime_objects]

        # Apply zero-phase filter directly to raw data
...
def generate_html_report(all_results, test_symbols, analysis_params, sort_by="signal_age"):
    ...
    np.mean([r["total_bars"] for r in all_results]) if all_results else 0
...
"""
    ...
    try:
        ...
        if buy_signals:
            print(
                f"🎯 Top buy opportunity: {buy_signals[0]['symbol']} (+{buy_signals[0]['price_change_pct']:.2f}%)"
            )
...
def plot_single_symbol(results):
    ...
    if peaks:
        ...
        for i, (idx, price) in enumerate(zip(peak_indices, peak_original_prices, strict=False)):
            ax.annotate(
                f"P{i + 1}: ${price:.4f}",
                (idx, price),
                xytext=(8, 18),
                ...
                arrowprops={"arrowstyle": "->", "color": bootstrap_colors["warning"], "alpha": 0.7},
            )
    ...
    if troughs:
        ...
        for i, (idx, price) in enumerate(zip(trough_indices, trough_original_prices, strict=False)):
            ax.annotate(
                f"T{i + 1}: ${price:.4f}",
                (idx, price),
                xytext=(8, -22),
                ...
                arrowprops={"arrowstyle": "->", "color": bootstrap_colors["success"], "alpha": 0.7},
            )
    ...
    ax.tick_params(
        axis="both", which="minor", colors=bootstrap_colors["secondary"], width=1, length=4
    )
    ...
    if company_name != symbol:
        title = (
            f"{company_name} ({symbol}) - Professional Technical Analysis\n"
            f"Bars: {total_bars} | Filter: Hanning(w={window_len}) | Lookahead: {lookahead} | Peaks: {len(peaks)} | Troughs: {len(troughs)} | Last: {last_time_str}"
        )
    else:
        title = (
    ...
    ax.grid(True, linestyle=":", alpha=0.3, color=bootstrap_colors["secondary"], linewidth=1)
    ...
    # Intelligent auto-placement of legend and stats box
    def find_best_legend_position(ax, original_prices, sample_indices):
        """Find the best position for legend and stats to avoid data overlap"""

        # Divide plot into regions and calculate data density
        total_samples = len(sample_indices)

        # Define only corner regions for legend placement (user requirement)
        corner_regions = {
            "upper_left": (0.0, 0.33, 0.66, 1.0),  # x: 0-33%, y: 66-100%
            "upper_right": (0.66, 1.0, 0.66, 1.0),  # x: 66-100%, y: 66-100%
            "lower_left": (0.0, 0.33, 0.0, 0.33),  # x: 0-33%, y: 0-33%
            "lower_right": (0.66, 1.0, 0.0, 0.33),  # x: 66-100%, y: 0-33%
        }

        # All regions for stats box placement

        # Calculate data density in each region
        y_min, y_max = original_prices.min(), original_prices.max()
        y_range = y_max - y_min

        def calculate_region_density(regions_dict):
            region_scores = {}
            for region_name, (x_min, x_max, y_min_norm, y_max_norm) in regions_dict.items():
    ...
    legend_region, stats_region = find_best_legend_position(ax, original_prices, sample_indices)

    # Map regions to matplotlib positions with precise corner placement

    # Alternative stats positions when legend is in a corner

    # Professional corner positioning with proper spacing from borders
    stats_gap_x = 0.02  # Stats box horizontal gap from left border
    stats_gap_y = 0.04  # Stats box vertical gap from top/bottom border (moved down/up)
    legend_gap_x = (
        0.02  # Legend horizontal gap from right border (reverted to fit nicely in corner)
    )
    ...
    if legend_region == "upper_right":
        ...
        legend_bbox = (1.0 - legend_gap_x, legend_gap_y)
...
def print_summary(results):
    ...
    print(
        "   • Price range: ${:.4f} - ${:.4f}".format(
            float(results["original_prices"].min()), float(results["original_prices"].max())
        )
    )
...
def main():
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate HTML report for batch analysis results"
    )
    ...
    try:
        ...
        if args.report:
            ...
            generate_html_report(all_results, test_symbols, analysis_params, args.sort_by)
