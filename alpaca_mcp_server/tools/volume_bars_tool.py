    ...
    try:
        ...
        if volume_threshold is None and auto_calculate_threshold:
            bars_request = StockBarsRequest(
                symbol_or_symbols=symbol, start=start_date, end=end_date, timeframe=TimeFrame.Day
            )
            ...
            if symbol in daily_bars:
                ...
                if not bars_df.empty:
                    ...
                    logger.info(
                        f"Auto-calculated volume threshold: {volume_threshold:,.0f} shares/bar"
                    )
                else:
        ...
        if use_bars:
            ...
            bars_request = StockBarsRequest(
                symbol_or_symbols=symbol, start=start_date, end=end_date, timeframe=timeframe
            )
            ...
            for timestamp, bar_row in bars_df.iterrows():
                ...
                bar = type(
                    ...
                    (),
                    {
                        ...
                        "trade_count": bar_row.get("trade_count", 1),
                        "timestamp": timestamp,
                    },
                )()
        ...
        output.append(
            f"Data Source: {'Bar Data (' + bar_timeframe + ')' if use_bars else 'Trade Data'}"
        )
        if use_bars:
        ...
        if stats:
            ...
            output.append(
                f"  • Avg Volume/Bar: {format_number(stats.get('avg_volume_per_bar', 0))}"
            )
            ...
            output.append(
                f"  • Avg Dollar Volume: {format_currency(stats.get('avg_dollar_volume', 0))}"
            )
            output.append(
                f"  • Avg Volume Imbalance: {format_percentage(stats.get('avg_volume_imbalance', 0))}"
            )
            output.append("")
        ...
        if not df.empty:
            ...
            for _, bar_row in df.tail(5).iterrows():
                ...
                output.append(
                    f"    Volume: {format_number(bar_row['volume'])} | "
...
async def start_volume_bar_streaming(
                ...
                try:
                    ...
                    if symbol in daily_bars:
                        ...
                        if not bars_df.empty:
                            ...
                            logger.info(
                                f"Auto-calculated threshold for {symbol}: {threshold_dict[symbol]:,.0f}"
                            )
                except Exception as e:
    ...
    aggregator = MultiSymbolVolumeBarAggregator(
        symbol_thresholds=threshold_dict, lookback_bars=100, on_bar_complete=on_bar_complete
    )
    ...
    if not _stock_stream_active:
        return (
            "⚠️ Stock stream is not active. Please start it first with:\n"
            "start_global_stock_stream(symbols='"
            + ",".join(symbol_list)
            + "', data_types=['trades'])"
        )
...
async def get_volume_bar_stats(symbol: str = None) -> str:
    ...
    symbols = [symbol] if symbol else list(_volume_bar_aggregators.keys())
    ...
    for sym in symbols:
        ...
        if stats:
            ...
            output.append(
                f"  Avg Imbalance: {format_percentage(stats.get('avg_volume_imbalance', 0))}"
            )
        ...
        if aggregator.current_volume > 0:
            ...
            output.append(
                f"  Current Bar Progress: {progress:.1f}% "
        ...
        if sym in _completed_volume_bars and _completed_volume_bars[sym]:
            ...
            if recent_bars:
                ...
                for bar in recent_bars:
                    output.append(
                        f"    {bar.timestamp_close.strftime('%H:%M:%S')}: "
...
async def compare_bar_types(
    ...
    try:
        ...
        time_stats = {
            ...
            "autocorr_lag1": (
                time_bars_df["returns"].autocorr(lag=1) if len(time_bars_df) > 1 else None
            ),
            "avg_volume": time_bars_df["volume"].mean(),
        }

        # Generate volume bars for comparison
        await get_volume_bars_from_history(
            symbol=symbol,
        ...
        if volume_stats:
            ...
            output.append(
                f"  • Avg Volume/Bar: {format_number(volume_stats.get('avg_volume_per_bar', 0))}"
            )
            output.append(
                f"  • Return Volatility: {format_percentage(volume_stats.get('returns_std', 0))}"
            )
            ...
            if volume_stats.get("autocorrelation_lag1") is not None:
                ...
                if abs(volume_stats["autocorrelation_lag1"]) < abs(
                    time_stats.get("autocorr_lag1", 1)
                ):
        ...
        if volume_stats and time_stats:
            ...
            if vol_autocorr < time_autocorr:
                output.append(
                    "  ✅ Volume bars show lower autocorrelation (better statistical properties)"
                )
