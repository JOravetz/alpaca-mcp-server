...
async def lifespan(app: FastAPI) -> None:  # type: ignore[misc]
    ...
    if hasattr(mcp_server, "_tool_manager") and hasattr(mcp_server._tool_manager, "tools"):
        ...
        from ..tools.help_tools import get_all_tools_help  # type: ignore[import-not-found]
    ...
    for tool_name in tools:
        ...
        tool_func = None
        for _prefix, module in tool_modules.items():
    ...
    if hasattr(mcp_server, "_resource_manager") and hasattr(
        mcp_server._resource_manager, "resources"
    ):
    ...
    for resource_uri, resource_func in resources.items():
        resource_registry[resource_uri] = {
            ...
            "description": (
                resource_func.description
                if hasattr(resource_func, "description")
                else "No description"
            ),
            "uri": resource_uri,
        }
    ...
    for prompt_name, prompt_func in prompts.items():
        prompt_registry[prompt_name] = {
            ...
            "description": (
                prompt_func.description if hasattr(prompt_func, "description") else "No description"
            ),
            "arguments": prompt_func.arguments if hasattr(prompt_func, "arguments") else [],
        }

    logger.info(
        f"✅ MCP Execution Service V2 ready with {len(tool_registry)} tools, {len(resource_registry)} resources, {len(prompt_registry)} prompts"
    )
...
async def root():
    ...
    <html>
        ...
        + """</div>
                ...
                <div class="stat-card">
                    <div class="stat-number">"""
        + str(len(resource_registry))
        + """</div>
                    <div>Resources Available</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">"""
        + str(len(prompt_registry))
        + """</div>
...
async def list_all_tools():
    ...
    for name, info in tool_registry.items():
        tools.append({"name": name, "description": info["description"], "schema": info["schema"]})
...
async def list_all_resources():
    ...
    for uri, info in resource_registry.items():
        resources.append({"uri": uri, "description": info["description"]})
    return {"resources": resources, "count": len(resources)}


@app.get("/api/execute/resource/{resource_uri:path}")
async def execute_resource(resource_uri: str) -> None:
    ...
    try:
        ...
        return {  # type: ignore[return-value]
            ...
            "execution_time": execution_time,
            "timestamp": datetime.now(UTC).isoformat(),
            "metadata": {"resource_uri": resource_uri, "traceback": traceback.format_exc()},
        }
...
async def execute_prompt(prompt_name: str, request: PromptExecutionRequest) -> Any:
    ...
    try:
        ...
        return {
            ...
            "execution_time": execution_time,
            "timestamp": datetime.now(UTC).isoformat(),
            "metadata": {"prompt_name": prompt_name, "arguments": request.arguments},
        }
