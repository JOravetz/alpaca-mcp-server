...
from ..server_components.tool_registrations import register_all_tools
from ..server_components.resource_registrations import register_all_resources
from ..server_components.prompt_registrations import register_all_prompts
...
async def execute_function_safely(func, **kwargs) -> ExecutionResponse:
...
async def list_all_tools() -> List[ToolSchema]:
    ...
    tool_definitions = {
        # Account Tools
        "get_account_info": (account_tools.get_account_info, "Get current account information including balances and status"),
        "get_positions": (account_tools.get_positions, "Get all current positions in the portfolio"),
        "get_open_position": (account_tools.get_open_position, "Get details for a specific open position"),
        ...
        "get_stock_snapshots": (market_data_tools.get_stock_snapshots, "Get comprehensive market snapshots"),
        "get_stock_bars": (market_data_tools.get_stock_bars, "Get historical price bars"),
        "get_stock_bars_intraday": (market_data_tools.get_stock_bars_intraday, "Get intraday historical bars with analysis"),
        "get_stock_trades": (market_data_tools.get_stock_trades, "Get recent trades for a stock"),
        "get_stock_latest_trade": (market_data_tools.get_stock_latest_trade, "Get the latest trade for a stock"),
        "get_stock_latest_bar": (market_data_tools.get_stock_latest_bar, "Get the latest minute bar for a stock"),
        
        # Market Info Tools
        "get_market_clock": (market_info_tools.get_market_clock, "Get current market status and next open/close times"),
        "get_market_calendar": (market_info_tools.get_market_calendar, "Get market calendar for specified date range"),
        
        # Options Tools
        "get_option_contracts": (options_tools.get_option_contracts, "Get option contracts for underlying symbol"),
        "get_option_latest_quote": (options_tools.get_option_latest_quote, "Get latest quote for an option contract"),
        "get_option_snapshot": (options_tools.get_option_snapshot, "Get comprehensive option snapshot with Greeks"),
        
        # Technical Analysis Tools
        "get_stock_peak_trough_analysis": (peak_trough_analysis_tool.analyze_peaks_and_troughs, "Get stock peak and trough analysis for day trading signals"),
        "analyze_peaks_troughs_fast": (c_peak_trough_wrapper.analyze_peaks_troughs_fast, "Ultra-fast peak/trough analysis using C implementation"),
        "compare_peak_trough_implementations": (c_peak_trough_wrapper.compare_implementations, "Compare performance between C and Python implementations"),
        
        # Volume Bar Tools
        "get_volume_bars_from_history": (volume_bars_tool.get_volume_bars_from_history, "Generate volume bars from historical data"),
        "compare_bar_types": (volume_bars_tool.compare_bar_types, "Compare statistical properties of time bars vs volume bars"),
        "start_volume_bar_streaming": (volume_bars_tool.start_volume_bar_streaming, "Start real-time volume bar aggregation"),
        "get_volume_bar_stats": (volume_bars_tool.get_volume_bar_stats, "Get current volume bar statistics and recent bars"),
        
        # Day Trading Scanner Tools
        "scan_day_trading_opportunities": (day_trading_scanner.scan_day_trading_opportunities, "Scan for explosive day-trading opportunities"),
        "scan_explosive_momentum": (day_trading_scanner.scan_explosive_momentum, "Quick scanner for explosive momentum moves"),
        "scan_after_hours_opportunities": (after_hours_scanner.scan_after_hours_opportunities, "Scan for after-hours trading opportunities"),
        "analyze_market_activity_fast": (c_stock_analyzer_wrapper.analyze_market_activity_fast, "Ultra-fast market activity analysis using C implementation"),
        "scan_explosive_stocks_fast": (c_stock_analyzer_wrapper.scan_explosive_stocks_fast, "Lightning-fast scan for explosive penny stocks"),
        "compare_analyzer_performance": (c_stock_analyzer_wrapper.compare_analyzer_performance, "Compare performance between C and Python analyzers"),
        
        # Streaming Analytics
        "get_enhanced_streaming_analytics": (streaming_tools.get_enhanced_streaming_analytics, "Enhanced streaming analytics with real-time calculations"),
        
        # P&L Analysis
        "get_single_day_pnl": (single_day_pnl.get_single_day_pnl, "Calculate P&L for a single specific trading day only"),
        
        # Watchlist Tools
        "create_watchlist": (watchlist_tools.create_watchlist, "Create a new watchlist with specified symbols"),
        ...
        # Asset Tools
        "get_all_assets": (asset_tools.get_all_assets, "Get all available assets with optional filtering"),
        "get_asset_info": (asset_tools.get_asset_info, "Get detailed information about a specific asset"),
        
        # Corporate Action Tools
        "get_corporate_announcements": (corporate_action_tools.get_corporate_announcements, "Get corporate action announcements"),
        
        # Streaming Tools
        "start_global_stock_stream": (streaming_tools.start_global_stock_stream, "Start global real-time stock data stream"),
        "stop_global_stock_stream": (streaming_tools.stop_global_stock_stream, "Stop the global stock streaming session"),
        "add_symbols_to_stock_stream": (streaming_tools.add_symbols_to_stock_stream, "Add symbols to existing stock stream"),
        "get_stock_stream_data": (streaming_tools.get_stock_stream_data, "Get streaming data for analysis"),
        "list_active_stock_streams": (streaming_tools.list_active_stock_streams, "List all active streaming subscriptions"),
        "get_stock_stream_buffer_stats": (streaming_tools.get_stock_stream_buffer_stats, "Get detailed streaming buffer statistics"),
        "clear_stock_stream_buffers": (streaming_tools.clear_stock_stream_buffers, "Clear streaming buffers to free memory"),
        
        # Stream-Aware Tools
        "stream_aware_price_monitor": (streaming_tools.stream_aware_price_monitor, "Enhanced real-time price monitoring"),
        "stream_optimized_order_placement": (streaming_tools.stream_optimized_order_placement, "Place order using optimal pricing from stream"),
        ...
        "place_option_market_order": (order_tools.place_option_market_order, "Place single or multi-leg options market order"),
        
        # Monitoring Tools
        "start_hybrid_monitoring": (monitoring_tools.start_hybrid_monitoring, "Start the hybrid trading monitoring service"),
        "stop_hybrid_monitoring": (monitoring_tools.stop_hybrid_monitoring, "Stop the hybrid trading monitoring service"),
        "get_hybrid_monitoring_status": (monitoring_tools.get_hybrid_monitoring_status, "Get monitoring service status"),
        "verify_monitoring_active": (monitoring_tools.verify_monitoring_active, "Verify monitoring is running with proof"),
        "add_symbols_to_watchlist": (monitoring_tools.add_symbols_to_watchlist, "Add symbols to monitoring watchlist"),
        "remove_symbols_from_watchlist": (monitoring_tools.remove_symbols_from_watchlist, "Remove symbols from watchlist"),
        "get_current_watchlist": (monitoring_tools.get_current_watchlist, "Get current monitoring watchlist"),
        "get_current_trading_signals": (monitoring_tools.get_current_trading_signals, "Get current trading signals"),
        "get_profit_spike_alerts": (monitoring_tools.get_profit_spike_alerts, "Get latest profit spike alerts"),
        "check_positions_after_order": (monitoring_tools.check_positions_after_order, "Force position check after order execution"),
        "ping_monitoring_service": (monitoring_tools.ping_monitoring_service, "Ping monitoring service for health check"),
        "get_monitoring_alerts": (monitoring_tools.get_monitoring_alerts, "Get recent monitoring alerts"),
        
        # FastAPI Monitoring Tools
        "start_fastapi_monitoring_service": (fastapi_monitoring_tools.start_fastapi_monitoring_service, "Start FastAPI monitoring service"),
        "stop_fastapi_monitoring_service": (fastapi_monitoring_tools.stop_fastapi_monitoring_service, "Stop FastAPI monitoring service"),
        "get_fastapi_monitoring_status": (fastapi_monitoring_tools.get_fastapi_monitoring_status, "Get FastAPI monitoring status"),
        "add_symbols_to_fastapi_watchlist": (fastapi_monitoring_tools.add_symbols_to_fastapi_watchlist, "Add symbols to FastAPI watchlist"),
        "remove_symbols_from_fastapi_watchlist": (fastapi_monitoring_tools.remove_symbols_from_fastapi_watchlist, "Remove symbols from FastAPI watchlist"),
        "get_fastapi_positions": (fastapi_monitoring_tools.get_fastapi_positions, "Get positions from FastAPI service"),
        "check_positions_after_order_fastapi": (fastapi_monitoring_tools.check_positions_after_order_fastapi, "Check positions after order via FastAPI"),
        "get_fastapi_signals": (fastapi_monitoring_tools.get_fastapi_signals, "Get trading signals from FastAPI service"),
        
        # Extended Hours Tools
        "get_extended_market_clock": (enhanced_market_clock.get_extended_market_clock, "Enhanced market clock with pre/post sessions"),
        "validate_extended_hours_order": (extended_hours_orders.validate_extended_hours_order, "Validate extended hours order"),
        "place_extended_hours_order": (extended_hours_orders.place_extended_hours_order, "Place order with automatic extended hours detection"),
        "get_extended_hours_info": (extended_hours_orders.get_extended_hours_info, "Get extended hours trading information"),
        
        # Advanced Plotting Tools
        "generate_advanced_technical_plots": (advanced_plotting_tool.generate_advanced_technical_plots, "Generate professional technical analysis plots"),
        "generate_stock_plot": (plot_py_tool.generate_stock_plot, "Generate stock analysis plots using plot.py"),
        
        # Help and Debug Tools
        "cleanup": (cleanup_tool.cleanup_server, "Clean up unnecessary temporary files"),
        "list_cleanup_candidates": (cleanup_tool.list_cleanup_candidates, "List files that can be cleaned up"),
    }
...
async def list_all_resources() -> List[ResourceSchema]:
    ...
    resource_definitions = {
        "account://status": (account_resources.get_account_status, "Real-time account health and trading capacity"),
        "positions://current": (position_resources.get_current_positions, "Live position data with P&L updates"),
        "positions://intraday_pnl": (intraday_pnl.get_intraday_pnl, "Track today's intraday P&L"),
        "market://conditions": (market_resources.get_market_conditions, "Current market status and conditions"),
        "market://momentum": (market_momentum.get_market_momentum, "Market momentum analysis"),
        "data://quality": (data_quality.get_data_quality_check, "Data quality and latency metrics"),
        "server://health": (server_health.get_server_health, "Server health and performance metrics"),
        ...
        "streaming://status": (streaming_resources.get_streaming_status, "Real-time streaming status"),
        "streaming://buffers": (streaming_resources.get_buffer_status, "Streaming buffer statistics"),
        "streaming://activity": (streaming_resources.get_activity_summary, "Recent streaming activity"),
        "portfolio://summary": (portfolio_resources.get_portfolio_summary, "Portfolio summary and performance"),
        "portfolio://risk": (portfolio_resources.get_risk_metrics, "Portfolio risk analysis"),
        "help://tools": (help_system.get_tools_help, "Available tools help"),
        "help://prompts": (help_system.get_prompts_help, "Available prompts help"),
    }
...
async def list_all_prompts() -> List[PromptSchema]:
    ...
    prompt_definitions = {
        "list_trading_capabilities": (list_trading_capabilities.list_trading_capabilities, "List all trading capabilities with guided workflows"),
        "account_analysis": (account_analysis_prompt.account_analysis, "Complete portfolio health check with actionable insights"),
        "position_management": (position_management_prompt.position_management, "Strategic position review and optimization"),
        "market_analysis": (market_analysis_prompt.market_analysis, "Real-time market analysis with trading opportunities"),
        "startup": (startup_prompt.startup, "Execute comprehensive day trading startup checks"),
        "day_trading_workflow": (day_trading_workflow.day_trading_workflow, "Complete day trading workflow execution"),
        "master_scanning_workflow": (master_scanning_workflow.master_scanning_workflow, "Master scanning workflow for opportunity detection"),
        "market_session_workflow": (market_session_workflow.market_session_workflow, "Market session analysis and workflow"),
        "options_strategy": (options_strategy_prompt.options_strategy, "Options trading strategy analysis"),
        "order_strategy": (order_strategy_prompt.order_strategy, "Order placement strategy optimization"),
        "portfolio_review": (portfolio_review_prompt.portfolio_review, "Comprehensive portfolio review and analysis"),
        "pro_technical_workflow": (pro_technical_workflow.pro_technical_workflow, "Professional technical analysis workflow"),
        "risk_management": (risk_management_prompt.risk_management, "Risk management analysis and recommendations"),
        "stream_centric_trading": (stream_centric_trading_prompt.stream_centric_trading, "Stream-centric trading workflow"),
        "stock_news_analysis": (stock_news_prompt.stock_news_analysis, "Stock news analysis and impact assessment"),
        "tools_reference": (tools_reference_prompt.list_all_tools, "Complete tools reference and usage guide"),
    }
...
async def execute_tool(tool_name: str, request: ToolExecutionRequest) -> ExecutionResponse:
    ...
    tool_functions = {
        ...
        # Volume Bar Tools
        "get_volume_bars_from_history": enhanced_volume_bars.get_volume_bars_from_history,
        "compare_bar_types": enhanced_volume_bars.compare_bar_types,
        "start_volume_bar_streaming": enhanced_volume_bars.start_volume_bar_streaming,
        "get_volume_bar_stats": enhanced_volume_bars.get_volume_bar_stats,
        ...
        # Streaming Tools
        "get_enhanced_streaming_analytics": streaming_tools.get_enhanced_streaming_analytics,
...
async def get_resource(resource_uri: str, request: ResourceRequest = None) -> ExecutionResponse:
    ...
    resource_functions = {
        ...
        "data://quality": data_quality.get_data_quality_check,
        ...
        "streaming://status": streaming_resources.get_streaming_status,
        "streaming://buffers": streaming_resources.get_buffer_status,
        "streaming://activity": streaming_resources.get_activity_summary,
        "portfolio://summary": portfolio_resources.get_portfolio_summary,
        "portfolio://risk": portfolio_resources.get_risk_metrics,
    }
...
async def execute_prompt(prompt_name: str, request: PromptExecutionRequest) -> ExecutionResponse:
    ...
    prompt_functions = {
        "list_trading_capabilities": list_trading_capabilities.list_trading_capabilities,
        ...
        "day_trading_workflow": day_trading_workflow.day_trading_workflow,
        "master_scanning_workflow": master_scanning_workflow.master_scanning_workflow,
        "market_session_workflow": market_session_workflow.market_session_workflow,
        "options_strategy": options_strategy_prompt.options_strategy,
        "order_strategy": order_strategy_prompt.order_strategy,
        "portfolio_review": portfolio_review_prompt.portfolio_review,
        "pro_technical_workflow": pro_technical_workflow.pro_technical_workflow,
        "risk_management": risk_management_prompt.risk_management,
        "stream_centric_trading": stream_centric_trading_prompt.stream_centric_trading,
        "stock_news_analysis": stock_news_prompt.stock_news_analysis,
        ...
        while True:
            ...
            try:
                ...
                if request_type == "execute_tool":
                    ...
                    response = await execute_tool(tool_name, ToolExecutionRequest(parameters=parameters))
                    ...
                    response = await get_resource(resource_uri, ResourceRequest(parameters=parameters))
                    ...
                    response = await execute_prompt(prompt_name, PromptExecutionRequest(arguments=arguments))
